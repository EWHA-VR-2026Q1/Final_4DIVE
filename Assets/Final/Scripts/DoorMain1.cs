...
public class DoorMain : MonoBehaviour
{
    ...
    void Start()
    {
        // 태그 대신 CharacterController로 플레이어 탐색
        var cc = FindFirstObjectByType<CharacterController>();
        if (cc != null) _player = cc.transform;
        Debug.Log($"[DoorMain] {gameObject.name} Start. targetScene={targetScene}, radius={triggerRadius}, player={(_player != null ? _player.name : "NULL")}");
    }
    ...
    void Update()
    {
        ...
        {
            _triggered = true;
            Debug.Log($"[DoorMain] 씬 전환 → {targetScene}  (dist={dist:F2})");
